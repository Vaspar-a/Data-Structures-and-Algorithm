...
class MinHeap{
	...
	void heapify(){
		...
		while(index >= 0){
			...
			if(rightChildIndex < length)
				smallestChild = minNode(getNode(leftChildIndex), getNode(rightChildIndex));	
			else if(leftChildIndex < length && rightChildIndex >= length)
				smallestChild = getNode(leftChildIndex);	
			else flag = false;
							
			parent = getNode(index);
			
			if(flag && smallestChild->data < parent->data){
				smallestChild->data = smallestChild->data + parent->data ;
				parent->data        = smallestChild->data - parent->data ;
				smallestChild->data = smallestChild->data - parent->data ;
				index++;
				continue;
			}
	...
	MinHeap(): head(NULL), tail(NULL), length(0) {}
